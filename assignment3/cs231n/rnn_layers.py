...
def affine_backward(dout, cache):
    ...
    """
    x, w, b = cache
    dx = dout.dot(w.T).reshape(x.shape) #dL/dx = dL/dy * dy/dx
    dw = x.reshape(x.shape[0], -1).T.dot(dout) #dL/dw = dL/dy * dy/dw
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(x.shape[1]):
        # Run forward pass, retrieve next_h and append new cache
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #Compute the full partial derivative of dnext_c and dprev_c
